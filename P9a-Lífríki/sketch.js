...
function setup() {
  ...
  for (var i = 0; i < 100; i = i+1){
     rubys[i] = new Ruby(random(200,400),random(100,400),random(50,200));
   }
}
...
function draw() {
  ...
  for (var i = 0; i < rubys.length; i = i+1){
   	rubys[i].move();
   	rubys[i].show();
  }
}
function mousePressed(){
  for (var i = rubys.length-1; i >= 0; i = i-1){
    if(fjarlægð(rubys[i].x,rubys[i].y,mouseX,mouseY) < 50){
		    rubys.splice(i,1);
	  }
  }
}
